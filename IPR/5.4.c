...
int iguales(vehículo *v1, vehículo *v2){
    for(int i = 0; i < 7; i++)
    {
        if (v1->matrícula[i] != v2->matrícula[i])
        {
            return 0;
        }
    }
...
int main(){
    ...
    if(iguales(&v1, &v2)) printf("Los dos coches registrados son el mismo\n");
    else printf("Los dos coches registrados son distintos\n");
